...
async function navigateTo(pageId) {
  ...
  // histórico do botão Voltar
  if (navHistory[navHistory.length - 1] !== pageId) navHistory.push(pageId);
  const backBtn = document.getElementById('back-btn');
  const isHome = pageId === 'dashboard' || pageId === 'aval-dashboard';
  backBtn.classList.toggle('hidden', isHome || navHistory.length <= 1);
...
function goBack() {
  if (navHistory.length > 1) {
    navHistory.pop(); // remove a página atual
